...
class helper_base extends helper_config {
  ...
  public static function add_resource($resource)
  {
    ...
    if (!array_key_exists($resource, self::get_resources()))
      watchdog('warning', "Resource $resource does
    ...
    if (self::$resource_list===null) {
      ...
      self::$resource_list = rray (
        'jquery' => array('javascript' => array(self::$js_pa(function_exists('iform_openlayers_get_file') ? iform_openlayers_get_file() : "OpenLayers.js"),
            self::$js_path."proj4js.js", self::$js_path."proj4defs.js", self::$js_path."lang/enenlayer' => array('javascript' => array(self::$js_path."OpenLayers.js", self::$js_path."proj4js.js", self::$js_path."proj4defs.j
       ...
       'indic// this resource is required for the georeferencer which dynamically constructs the resource name depending on the driver selected.
        ...
        'google_search' => array('deps' =>array('georeference_google_search_api'), 'javascript' => array(self::$js_path."google_search.js"eorference_google_search_api' => array('javascript' => array("http://www.google.com/jsapi?key=".parent::$google_search_api_key)),
        ...
        'jquery_ui' => array('deps' => array('jquery'), 'stylesheets' => array("$indicia_theme_path$indicia_theme/j
        'jquery_ui_fr'jqplot_trendline' => array('javascript'=>array(self::$js_path.'jqplot/plugins"jquery.ui.datepicker-fr.js")),
        'jquery_form'deps' => array(''), 'stylesheets' => array(self::$css_path."default_site.c"jquery.form.js")),
        'json' => array('javascript' => array(self::$js_path."json2.js")),ray(self::reportPicker' => array('deps' => array('treeview'), 'javascript' => array(self::$js_path."reportPickert' => a
        'treevieway('indiciaMapEdit'), 'javascript' => array(self::$js_path."jquery.indiciaMap.edit.lo:$js_path."json2.js")),
        'treeview' => array('deps' => array('jqu)),
        'treeview_async' => array('deps' => array('treeview'), 'javascript' => array(self::$js_path."jquery.treeview.async.js", self::$js_path."jquery.treeview.edit.js")),path."jquery.treeview.js", self::$js_path."jquery.treeview.async.js",
          /api/js?sensor=false"=> rray('java'multimap' => array('javascript' => array("http://developer.multimap.com/API/maps/1.2/".parent::$multimap_api_
        'fancyboxay('indiciaMapEdit'), 'javascript' => array(self::$js_path."jquery.ile_search.js"
            )
        ),
       ...
       'treeBrowser' => array('deps' => array('jquery','jquery_ui'), 'javascript' => array(self::$js_path."jquery.treebrowser.js")),
        ...
        'jqplot_bar' => array('javascript' => array(self::$js_path.'jqplot/plugins/jqplot.barRenderer.min.js')),s.min.js')),
        'jqplot_bar' => array('javascript' => array(self::$js_path.'jqplot/plugins/jqplot.barRenderr.min.js')),
...
$('.ui-state-default').live('mouseout', function() {
   ...
   *
   * @param string $fieldname Fieldname of the control to retrieve errors for. the
   * control with the supplied ID.
   *
   * @param string $fieldname Fieldname of the control to ret
   */
  ...
  public static function http_post($url, $postargs=null, $output_errors=true) {
    ...
    if (curl_errno($session) || $httpCode!==200) {
      ...
      $return = array(
          'result'=>false,   echo "Server response<br/>";
        echo $response.'</div>';
      }
   ...
   */
  public static function get_uploaded_image_folder() {that submitted images end up in according to the helper_config.
   */
  public stat {
   ...
   */
  public static function relative_client_helper_path() {lient_helpers folder from wherever the current PHP script is.
   */
  public static funct
  ...
  /**j < count($clientHelperFolder); $j++) {
      $path .= $clientHelperFolder[$j] . '/';
    
   * parameters for a process. Returns the HTML required for a parameters form, e.g. the form form used to specify the input
   * parameters for a process. Returns the HTML req
...
pt    $r .= "<input type=\"hidden\" name=\"$fieldPrefix$key\" value=\"tPself::get_preset_param($options, $key)key, class=\"".$fieldPrefix."idlist-param\" />\n";
    } elseif (isset($options['extraParams']) && array_key_exists($key, $options['extraParams'])) {
pt    $r .= "<input type=\"hidden\" name=\"$fieldPrefix$key\" value=\"tPself::get_preset_param($options, $key)key, $optio
...
options['presetParams'][$key]."\" />\n";
      ...
      if (isset($info['linked_to']) && isset($info['linked_filter_field'])) {
       ...
       $ctrlOptions['report'extraParams']) && array_key_exists($info['linked_to'], $options['extranked_filter_ield'])) {         if (isset($options['presetParams']) && array_key_exists($info['linked_to'], $options['presetParams'])) {
...
mapSettingsHooks.push(add_map_tools)\n";
  ...
  /**
   * Utility method that returns the parts required to build a link back to th
   */
   ...
   *
   * data services.
   *uploaded to the client website upload folder, and moves it to the warehouse upload folder using the
   * dat
   ...
   */
    ...
    if ($readAuth==null) $readAuth=$_POST;ouse($path, $persist_auth=false, $readAuth = null, $service='data/handle_media') {
    if ($readAuth==null) $
  ...
  */
  ...
  /**nce" name="nonce" type="hidden" class="hidden" ' .
        'value="'.$nonce.'" />'.
  ...
  * @param string $password Indicia password for the website.
  */
  public static function get_read_auth($website_id, $password) {ram string $password Indicia password for the website.
  */
  public static fu
    $postargs = "website_id=$website_id"; {
    self::$website_id = $website_id; /* Store this for use with data caching */
...
/**
  ...
  */
  public static function get_read_write_auth($website_id, $password) {ram string $password Indicia password for the website.
  */
  public static fu
    $postargs = "website_id=$website_id"; {
    self::$website_id = $website_id; /* Store this for use with data caching */
    write_nonces', $postargs);
   ...
   * depending on what is on your form. dump_header is not intlligent because the form is not
   * built yet, but placing links in the header leads to cleaner code 
   * in the documentation for the add_resource method. The default for this is jquery_ui and defaultStylesheet.
   *scribed
   * in the documentation for the add_resource method. The defa
   */
  ...
  * page which uses the data entry helper so output all JavaScript required by previous calls.call @param boolean $closure Set to true to close the JS with a function to ensure $ will refer to jQuery.d at the end of each HTML
  * page which uses the data entry helper so output all JavaScript require
  *
      ...
      foreach ($resources as $resource)
      {
        if (!in_array($resource, self::$dumped_resources)) {
          if (isset($resourceList[$resource]['stylesheets'])) {
            foreach ($resourceList[$resource]['stylesheets'] as $s) {  if (isset($resourceList[$resource]['stylesheets'])) {
            foreach ($resource
            }
          }
          ...
          $libraries .= '<script type="text/javascript" src="'.self::$js_path."indicia.functions.js\"></script>\n";
              self::$indiciaFnsDone = true;
                ...
                $libraries .= "<script type=\"text/javascript\" s
...
";
  ...
  **
  ...
  public static func
    ...
    if (self::$validated_form_id) {
      ...
      self::$javascript .= "
        ...
        errorClass: \"".$indicia_templates['error_class']."\",:$javascript .= "$('#".self::$validated_form_id."').validat" : "errorElement: 'p',") ."
   ...
   * prefix template, a label (if in the options), a control, the control's errors and a
   * 
   * @param array $options Options array containing the control replacement values for the templates.cia_templates variable.
  ...
  public static function apply_template($template, $options) {
    global $indicia_templates;the standard templates.
   */
  public static function apply_template($temp
...
s['fieldname']);
 ...
 /**at the end if that is the preferred position
    $r .= self::get_help_text($options, 'after')
  * validation rules defined.
  * To specify validation on each control, set the control's options arrayr any controls with
  * validation rules defined.
  * To specify validation on each control, se
 ...
 /**on enable_validation($form_id) {
   ...
   */
  protected static function get_help_text($options, $pos) {ted.
   * @return string Templated help text, or nothing.
   */
  prote
   ...
   */ate string (e.g. <div id="{id}">) and replaces the tokens with the equivalent valu
    // Build an array of all the possible tags we could replace in the template.
    $replaceTags=array();
    $replaceValues=array();
  ...
  * @todo Suspect there is a versioning issue, $optione, b
    ...
    foreach ($rules as $rule) {
      ...
      if    ($rule=='required'
          ...
          || $rule=='integer') {
        $converted[] = $rule.':true';
      // Now any rules which need parsing or conversion=='url'
    if ($rule=='date' && !isset($options['allowVagueDates']) || 
  ...
  * specified in the optionsverted);
  }

   /**
  * Returns a static template which is either a 
