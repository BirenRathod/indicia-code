...
$indicia_templates = array(
  ...
  'invalid_handler_javascript' => "ght: function(element, errorClass) {
          $(element).removeClass('ui-state-error');
        },
        invalidHandler: function(form, validator) {
          var tabselected=false;
          jQuery.eacvar ctrl = jQuery('[name=' + ctrlId.replace(/:/g, '\\\\:').replace(/\[/g, '\\\\[').replace(/\\]/g, '\\\\]') + ']');
            if (!tabselected) {
              var tp=ctrl.filter('input,select,textarea').closest('.ui-tabs-panel');
              if (tp.length===1) {
                $(tp).parent().tabs('select',tp.id);
              }s.tabs('select',jQuery('[name=' + ctrlId.replace(/:/g, '\\\\:') ctrl.parents('fieldset').removeClass('collapsed');
     ...
     if (self::$resource_list===null) {
      ...
      self::$resource_list = array (
        'jquery' => array('javascript' => array(self::$js_pa(function_exists('iform_openlayers_get_file') ? iform_openlayers_get_file() : "OpenLayers.js"),
            self::$js_path."proj4js.js", self::$js_path."proj4defs.js", self::$js_path."lang/enenlayers' => array('javascript' => array(self::$js_path."OpenLayers.js", self::$js_path."proj4js.js", self::$js_path."proj4defs.js")),
        'graticule' => array('deps' =>array('openlayers'), 'javascript' => array(self::$js_path."indiciaGraticule.js")),
        'clearLayer' =>deps' => array('validation'), > array('deps' =>array('openlayers'), 'javascript' => array(self::$js_path."clearLayer.js")),
        ...
        'google_search' => array('deps' =>array('georeference_google_search_api'), 'javascript' => array(self::$js_path."google_search.js"eoreference_google_search_api' => array('javascript' => array("http://www.google.com/jsapi?key=".parent::$google_search_api_key)),
        ...
        'autocomplete' => array('deps' => array('jquery'), 'stylesheets' =indicia_locks' => array('deps' =>array('jquery_cookie', 'json'), 'javascript' => array(self::$js_path."indicia.locks), 'stylesheets' => arraycookie' => array('deps' =>array('jquery'), 'javascript' => array(self::$js_path."jquery.cooki'), 'stylesheets' => array(self::$css_path."jquery.autocomplete.css"), 'javascript' => array(self::$js_path."jquery.autocomplete.js")),
        ...
        'jquery_ui_fr' => array('deps' => array('jquery_ui'), 'javascript' => array(self::reportPicker' => array('deps' => array('treeview'), 'javascript' => array(self::$js_path."reportPickert' => array(self::$js_path."jquery.ui.datepicker-fr.js")),
        'json' => array('javascript' => array(self::$js_path."json2.js")),
        'treeview' => array('deps' => array('jqu)),
        'treeview_async' => array('deps' => array('treeview'), 'javascript' => array(self::$js_path."jquery.treeview.async.js",eview.css"), 'javascript' => array(self::$js_path."jquery.treeview.js", self::$js_path."jquery.treeview.async.js",
          /api/js?sensor=false"=> array('java'multimap' => array('javascript' => array("http://developer.multimap.com/API/maps/1.2/".parent::$multimap_api_key)),
   pi?key=".parent::$google_search_api_key,
              self::$js_path."google_search.js"
            )
        ),
        'fancybox' => array('deps' => array('jquery'), 'stylesheets' => array(self::,
        ...
        'defaultStylesheet' => array('deps' => array(''), 'stylesheets' => array(self::$css_path."default_site.css"), 'jmetadata.js', self::$js_path.'jquery.validate.js', self::$js_path.'additional-methodst' => array()),
        ...
        'jqplot' => array('stylesheets' => array(self::$js_js',
                '[IE]'.self::$js_path.'jqplot/excanvas    
        ...
        'wizardprogress' => array('deps' => array('tabs'), 'stylesheets' => array(self::$css_path."wizard_progress.css"))deps' => array('jquery'), ),
        ...
        'complexAttrGrid' => array('javascript'=>array(self::$js_path."complexAttrGrid.jonwidget/jsonedit.js",
            self::$js_path."jquery.jso  
    /**
...
$('.ui-state-default').live('mouseout', function() {
  $(this).removeClass('ui-state-hover');
    ...
    self::$default_styles = trueuery.jsonwidget.js"), 'stylesheets'=>array(self::$css_path."jsonwidget.css"))
      );
    }
  ...
  public static function check_errors($fieldname, $plaintext=false)
  {
    ...
    if ($error!='') {
      if ($plaintext) {
        ...
        return self::apply_error_templa.
   ...
   * cUrl error number (as generated by the cUrl library used for the post).te($error, $fieldname);
      }
    } else {
      retur=nulln '';
    }
  ...
  public static function http_post($url, $postargs, $output_errors=true) {
    ...
    curif (is_array($postargs) && version_compare(phpversion(), '5.5.0') >= 0) {
    ...
    if ($postargs!==null) {
      curl_setopt ($session, CURLOPfalsOST, true);
      curl_setopt ($session, CURLOPT_POSTFIELDS, $postargs);
    }
    curl_setopt($session, CURLOPT_HEADER, true);
    curl_setopt($ses$httpCode = curl_getinfo($session, CURLINFO_HTTP_CODE); 
    // Check for an error, or check if the http response was not OK.
    if (curl_errno($session) || $httpCode!==200   if (curl_errno($session) || (strpos($response, 'HTTP/1.1 200 OK')===false && strpos($response, 'Connection: close')===false)) {
      if ($output_errors) {
        echo '<div URL:'.$url.'<div class="error">cUrl POST request failed. Please check cUrl is installed on the server and the $base_url setting is correct.<br/>';
  ...
  public stat {
      ...
      self::getRootFolder() . self::client_helper_path() . .(isset(self::$indicia_upload_path) ? se  
  /**
  ...
  public static function client_helper_path() {
    ...
    if (function_exists('iform_client_helpers_path')) 
      ...
      return $client_helper_pathload_path) ? self::$indicia_upload_path : 'upload/');
    else {
   ...
   * </li>
   * <li><b>form</b><br/>
   * Associative array definNameP form content.
   * </li>
   * <li><b>readAuth</b><br/>
   ...
   * <li><b>paramsToExclude<li><b>defaults</b><br/>
   * Associative array of default values for each form element.
   * </li>
   * <li><b>ignoreParams</b><br/>
   * An optional array of parameterextra for parameters that should be skipped in the form output despite being in the form definition.
   * </li>
   * <li><b>presetParams</b><br/>
   * Optionalli><b>inlineMapTools</b><br/>
   ...
   * simple parameter forms><br/>
   * Optionalli><b>nocache</b><br/>
   * Set to true to disable caching of lookups><br/>
   * Optional array of  @param boolean $hasVisibleContent On completion, this is set to true if there are visible
   * controls in the params form. If not, then it may be appropriate to skip the displaying of this
   * params form since it is not necessary.ay of param names and values that have a fixed value and are, &$hasVisibleContent) {
    require_once('data_entry_helper.php');
    $javascript = '';
    // track if there is anything other than hiddens on the form
    $hasVisibleContent=false  * </l// apply defaults
    ...
    /ul>
   */
  p=(isset($options['fieldNamePrefix']) ? $options['fieldNamePrefixrefix = (isset($opt    $r .= "<input type=\"hidden\" name=\"$fieldPrefix$key\" value=\"$default\" class=\"test\"/>\n";
      }
    }
  ...
  if (document.activeElement.id=='geom_buffe  $javascript .= "if (typeof $.fn.indiciaMapPanel!=='undefined') {\n";
        $fieldname=ic static function buiNamePrefix']) ? $options['fieldNamePrefix'].'-' : '') .$key;
        self::add_resource('spatialReports');
        self::add_resource('clearLayer');
  '#run-$javascript .= "  enableBuffering();\n";
        if ($options['inlineMapTools']) {
          $r .= '<label>'.$info['display'].':</label>';
    
        data_entry_helper::$javascript .= "enableBuffering();\n";
      $r .= '<label>'.$ctrlOption    '<div id="map-toolbar" class="olControlEditingToolbar left"></div></div><br/>';
        }
  ...
  ="orig-wkt" id="orig-wkt" '.
            'value="'.(isset($_POST['orig-wkt']) ? $_POST['orig  // to handle getting the polygons from the edit layer into the report parameter
  ...
  '#run-report').click(function(evt) {
  if (document.activeElement.id=='geom_buffe  $origWkt = empty($_POST['orig-wkt']) ? '' : $_POST['orig-wkt'];
        else
   ...
   */
  private static function get_params_form_control($key, $info, $options, &$tools) { </ul>r = ''; </ul> </ul>
   */
  p=(isset($options['fieldNamePrefix']) ? $options['fieldNamePrefixrefix = (isset($opt$ctrlOptions = array(
      'label' => $info['display'],
...
abel' => $info['display'],
        ...
        'fieldname' => $fieldPrefix.$keif (isset($options['defaults'][$key]))
...
pt    $r .= "<input type=\"hidden\" name=\"$fieldPrefix$key\" value=\"tPself::get_preset_param($options, $key)key, $options['pr} elseif ($info['datatype']=='lookup' && isset($info['population_call'])) {
options['presetParams'][$key]."\" />\n";
      ...
      if (isset($info['linked_to']) && isset($info['linked_filter_field'])) {
       $ctrlOptions['report'extraParams']) && array_key_exists($info['linked_to'], $options['extranked_filter_field'])) {         if (isset($options['presetParams']) && array_key_exists($info['linked_to'], $options['presetParams'])) {
...
o['linked_to']]['display']
        ...
        $r .= da// Convert the lookup values into an associative array
...
mapSettingsHooks.push(add_map_tools)\n";
  ...
  public static function get_reload_link_parts() {
    ...
    $gets = ($split!==false && strlen($_SERVER['REQUEST_URI']) > $sforeach ($gets as $get) {
      $tokens = explode('=', $get);
      // ensure a key without value in the URL gets an empty value
      if (count($tokens)===1) $tokens[] = '';
      $getsAssoc[$tokens[0]] = $tokens[1]ssoc = array();
    foreach ($gets as $get) {
  ...
  /**
   * Takes an associative array and converts it to a list of params for a query string. This is lik @param array $array Associative array to convert.
   * @param boolean $encodeValues Default false. Set to true to URL encode the values being added to the string.
   * @return string The query string is like
   ublicuild_query but it does not url encode the & separator, and gives control over urlencoding the array values.
   */
  protected static function array_to_query_string($array, $encodeValues=false) {
    ...
    /**
   * Applies a output template to an array. This is used tparams Array holding the parameters to merge into the templatelist,
   ...
   * @param boolean $allowHtml If param boolean $allowEscapeQuotes If true then parameter names can be suffixes -esape-quote, -escape-dblquote, 
   * -escape-htmlquote or -escape-htmldblquote to insert backslashes or html entities into the replacements for string escaping. Html If true then HTML is emitted as is from the parameter values inserted into the template,
   * otherwise they are escaped.
   * @return string HTML for the i, $allowEscapeQuotes the item label
   */
  public static function mergeParamsIntoTemplate($params, $template, $useTemplateAsIs=false, $allowHtml=false) {
    ...
    $replaceTags=array();
    $replaceValues=array();
    foreach ($params as $paraif ($allowEscapeQuotes) {
 ...
 /**
  * Internal function to // $_SERVER['SCRIPT_NAME'] can, in contrast to $_SERVER['PHP_SELF'], not
    // be modified by a visitor.
    if ($dir = trim(dirname($_SERVER['SCRIPT_NAME']), '\,/'))
      return "/$dir/";
    else
      return '/';    windows may just return a single backslash.
...
/**
  ...
  */
  ...
  public static function dump_header($resources=null) {
    ...
    foreach ($resources as $resource) {
      self::add_resource($resource)resources(self::$required_resources) . 
        self::get_scripts('$("body").addClass("js");', '', '', true output the resources
    return self::internal_dump_javascript('$("body").addClass("js");', '', '', self::$required_resources);
  }

  /**
  * Helper function to collect javascript code in a single location. Should be call @param boolean $closure Set to true to close the JS with a function to ensure $ will refer to jQuery.ed at the end of each HTML
  ...
  *
  * @link http://co$closure=falsede.google.com/p/indicia/wiki/TutorialBuildingBasicPage#Build_a_data_entry_page
  */
  public static function dump_javascript() {
    ...
    // Jquery validation js has to be added at this late stage, becresources(self::$required_resources);
    $dump .= self::get_scripts(self::$javascript, self::$late_javascript, self::$onload_javascript, true, $closure= self::internal_dump_javascript(self::$javascript, self::$late_javascript, self::$onload_javascript, self::$required_resources);
  ...
  /**
   * Internal implementation of the dump_javasparam array $resources List of resources to includeump_javascript method which takes the javascript and resources list
    ...
    if (isset($resources)) {
      ...
      foreach ($resources as $resource)
      {
        if (!in_array($resource, self::$dumped_resources)) {
          ...
          // Record the resource as beireturn $stylesheets.$libraries;
  }
   ...
   */
  public static function get_scripts($javascript, $late_javascript, $onload_javascript, $includeWrapper=false, $closure=false) {as being dumped, so we don't do it again.
...
";
  ...
  public static function setup_jquery_validation_js() {
    ...
    // We also define the error label to be wrapped in a 
        var validator = <p> if it is on a newline.
    if (self::$validated_foignore: \":hidden,.inactive\",lidated_form_id) {
      global $indicia_templates;
      self::$javascript .= "$('#".self::$validated_form_id."').validat" : "errorElement: 'p',") ."
          r_class']."\",
        ". (in_array('inline', self::$var jqElement = $(element);
        ...
        unhighli".$indicia_templates['invalid_handler_javascript']."          $('input[name=' + ctrlId.replace(/:/g, '\\\\:') + '].",".
        ...
        errorPlacement: function(error, element) {}") ').parents('.
  ...
  public static function apply_template($template, $options) {
    ...
    // Add a label only if specified in the options array. Link the lab otherwise the fieldname (as the fieldname control could be a hidden control).
    ...
    // Ou lock icon to the control if the lockable option is set to true
...
s['fieldname']);
    ...
    // Add suffix
    if (isset($validationClasses) && !emnosuffix', $options);
    }
    // Add an error icon'fieldname', $options)) {
        $error = self::check_errors($options['fieldname'], tru$options);
    ...
    if ($error && in_array('message', $options['validation_mode'])) {
      $r .=  self::apply_error_template($error, $options['f$r .= self::apply_static_template('suffix', $options);static_template('requiredsuffix', $options);
    } else {
    ...
    if (!empty($options['label']) && isset($indicia_templates['controlWrap'])) 
      ...
      ontain a help text, output it at the end if that is the preferred position
  ...
  * @param string @form_id Id of the form the validation is b//Only submit once if the user clicks submit button several times
    self::$javascript .= '
  ...
  */
  ...
  public static function get_termlist_terms($auth, $termlist, $filter=null) {
    ...
    return $termsc static function enable_validation($form_id) {
 ...
 /**
    ...
    foreach ($rules as $rule) {
      if (isset($custom_terms) && array_key_exists($options['fieldname'], $custom_terms))
        self::$validation_messages[$options['fieldname']][$rule] = sprintf(lang::get("validation_$rule"),
          lang::get($options, $options['fieldname']));
    }
  ...
  protected static function get_help_text($options, $pos) {
    $options = array_merge(array('helpTextClass'=>'helpText'), $opt$options['helpText'] = lang::get($options['helpText']);'), $options);
  ...
  /**
   * Takes a tem @param string $template The templatable string.
   * @param string $options The array of items which can be merged into the templates a template string (e.g. <div id="{id}">) and replaces the tokens with the equivalent values looked up from the $options array.
   */
  protected static function apply_replacements_to_template($template, $options) {
   ...
   /**
  * Takes a list of validation rules in Kohana/Indicia format, and converts them param array $options Options passed to the validated controlts them to the jQuery validation
  * plugin metadata format.
  * @param array $rules List of validation rules to be converted.
  * @return stri of validation rules.
  * @todo Suspect there is a versioning issue, $optione, because the minimum/maximum rule classes don't work.
  */
  protected static function convert_to_jquery_val_metadata($rules) {
    ...
    if ($rule=='date' && !isset($options['allowVagueDates']) || 
        ...
        $converted[] = 'number:true if ($rule=='date') {
        $converted[] = 'customDate';
      } else if ($rule=='digit') {
        $converted[] = 'digits';
      // the next test u-?ses a regexp named expression to find the digit in a mamax:'.$matches['val'] } elseif (preg_match('/maximum\[(?P<val>\d+)\]/', $rule, $matches)) {
        $conver-?ted[] = '{maxValue:'.$matches['val'].'}';
      // andmin:'.$matches['val'];
  ...
  */
  protected static function jq_esc($name) {
    ...
    $from = array(':','[',']');
    $to = array('\\\\:','\\\\[','\\\\]');
    return $name ? str_replace($from, $to, $name) : $nametemplate specified
      $r = $indicia_templates[$name];
    }
    return self::apply_repl @param string $error The error message.
   * @param string $fieldname The name of the field which the error is being attached toly_replacements_to_template($r, $options);
  }
  ...
  public function cache_set($cacheOpts, $toCache, $cacheTimeout=false) {
    ...
    $t  
  /**
   * Protected function to fetch a validated timeout value from passed in options arrayuired for a parameters form, e.g. the form
   * defined for input of report parameters or thecachetimeout</b><br/>
